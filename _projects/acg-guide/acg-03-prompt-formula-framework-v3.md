...
- Even perfect detailed prompts are limited by model prompt adherence
...
**Purpose:**
The block library (see `prompt-blocks.json`) defines available block types and their hierarchical structure. Values in the library remain **empty** - they are populated dynamically by LLMs based on user intent when generating prompts.

**What the library provides:**
- Predefined block paths and categories
- Hierarchical structure and organization
- Reference of available blocks for each use case
...
│   │   ├── Gender (Core Block)
│   │   │   └── Example values (filled by LLM): [male, female, non-binary]
│   │   ├── Age Group (Core Block)
│   │   │   └── Example values (filled by LLM): [young adult, middle-aged, mature]
│   │   ├── Ethnicity (Core Block)
│   │   │   └── Example values (filled by LLM): [caucasian, east asian, african, etc.]
│   ├── Physical Features (Block Group)
│   │   ├── Face Shape (Core Block)
│   │   │   └── Example values (filled by LLM): [oval, heart-shaped, square, round]
│   │   ├── Body Type (Core Block)
│   │   │   └── Example values (filled by LLM): [slim, athletic, curvy, plus-size]
├── Environment (Block Group)
│   ├── Setting Type (Core Block)
│   │   └── Example values (filled by LLM): [photo studio, indoor location, outdoor location]
│   ├── Location Style (Core Block)
│   │   └── Example values (filled by LLM): [minimalist, industrial, rustic, modern]
```
