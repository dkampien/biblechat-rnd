...
This comprehensive guide provides an in-depth framework for generating prompts for AI content generation models. It serves as a complete reference for understanding modalities, designing workflows, and mastering systematic prompting approaches.

**Purpose:** Deep technical reference for AI content generation - covers theory, technique, and application regardless of which specific models exist.

**For a quick, practical introduction:** See [Quick Start Guide](../ai-content-quick-start.md)
...
> **New to AI content generation?** See the [Quick Start Guide](../ai-content-quick-start.md) for a practical, workflow-first introduction. This comprehensive guide provides deep reference and detailed technique.

This guide covers:
- **Single generations**: One modality, one prompt
- **Workflows**: Multiple modalities chained together

**Terminology:**
- **Goal** = Overall outcome you want to achieve (e.g., "30-second video of a model at the gym")
- **Intent** = What content to create in a specific generation step (e.g., "image of person at bench press")

> For detailed application guidance, see **"Applying This Framework"** at the end of this guide.
...
### Foundation
- [1. Understanding AI Models](#1-understanding-ai-models)
- [2. Modalities: Model Capabilities](#2-modalities-model-capabilities)
- [3. AI Generation Workflows](#3-ai-generation-workflows)

### Technique
- [4. Prompts & Modalities](#4-prompts--modalities)
- [5. Formulas & Blocks](#5-formulas--blocks)
- [6. Detail Levels](#6-detail-levels)
...
- [Applying This Framework](#applying-this-framework)

---

## 1. Understanding AI Models

**Summary:** Learn why this framework focuses on capabilities (modalities) rather than specific model names, and understand the difference between models and platforms.
...
**Summary:** Comprehensive reference of AI generation capabilities - image, video, and 3D modalities with their input→output patterns.
...
- **image-to-image_edit (i2i_edit)** - text (prompt targeting) -> edited image
- **image-to-image_reference (i2i_reference)** - text + multiple references or objects 
...
- **image-to-video_lipsync (v2v)** - text + reference image + voice audio
- **video-to-video (v2v)** - Video + text → transformed video
- **video-to-video_lipsync (v2v)** - text + reference video + voice audio
...
**Summary:** Learn how to chain modalities into multi-step workflows for complex outputs, including frame chaining and multi-scene content strategies.
...
### Using This Guide for Workflows

When working with workflows, break down your goal into individual steps, then apply the single generation process (modality + intent) to each step. The output of one step often becomes the input (intent) for the next.

> See **"Applying This Framework"** at the end of this guide for detailed workflow process.
...
**Summary:** Understand how different modalities require fundamentally different prompts. Learn the three-component approach: Focus, Phrasing, Formula.
...
**Summary:** Introduction to systematic prompt templates (formulas) and their atomic components (blocks) for structured prompt generation.
...
**Summary:** Learn how to control prompt depth by adjusting detail levels while maintaining the same formula structure.
...
**Summary:** Master modern prompting techniques, understand model-specific considerations, and learn about prompt adherence variations across models.
...
**Summary:** See the framework applied to real workflows - from simple controlled videos to complex character consistency across multiple scenes.
...
#### Getting Modality and Intent

**Interaction Pattern:**
- If user provides both modality and intent → proceed
- If something is missing → ask for it

**Understanding Intent:**

Intent should describe whatever the modality generates:
- If the modality has input (image, video): describe what changes/happens
- If the modality generates from scratch: describe what should exist

#### Generation Process

1. **Identify modality and intent** - Use interaction pattern above to get both
2. **Choose prompting approach** - Refer to Section 4 for modality-specific focus and phrasing
3. **Select appropriate formula** - Use block structure suited for the modality
4. **Fill blocks based on intent** - Populate blocks with content from user's intent
5. **Test and refine** - Generate, evaluate, adjust detail level and block values
6. **Document** - Capture what works for future use (use output formats from framework)

### For Workflows

When working with multi-step workflows:

1. **Define goal** - Establish overall outcome (e.g., "30-second video with consistent character")
2. **Break down into steps** - Identify modalities needed, in sequence (see Section 3 for workflow guidance)
3. **For each step** - Apply the single generation process above (modality + intent → blocks → prompt)
4. **Chain outputs** - Use output from one step as input (intent) for the next
5. **Test workflow** - Generate each step, evaluate continuity and quality
6. **Iterate** - Refine individual steps or workflow structure based on results
7. **Document** - Capture workflow patterns that work for future reuse
